  ...
  >
    <div v-if="noticeStore.activeNotice" class="notice-detail">
      <div class="notice-detail__info">
        ...
        <div class="notice-detail__info-item">
          <span class="label">接收时间</span>
          <span>{{ formatDateTime(noticeStore.activeNotice.receivedAt || noticeStore.activeNotice.publishedAt) }}</span>
        </div>
        <div class="notice-detail__info-item">
          <span class="label">状态</span>
          <el-tag
            :type="noticeStore.activeNotice.read ? 'success' : 'danger'"
            size="small"
          >
            {{ noticeStore.activeNotice.read ? '已读' : '未读' }}
          </el-tag>
        </div>
        <div v-if="noticeStore.activeNotice.read && noticeStore.activeNotice.readAt" class="notice-detail__info-item">
          <span class="label">阅读时间</span>
